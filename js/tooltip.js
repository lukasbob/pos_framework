(function($) {
	...
	$.fn.tt = function(options) {
		...
		return this.each(function() {
			...
			$this.isOn = false;
			var TT_ID = '#' + o.ttIdPrefix + $this.attr('id');			
			...
			if (!o.get && ($this.attr('id').length === 0 || !($(TT_ID)[0]))) {			
			...
			function getTooltipPosition() {
				...
				pos = { //All the possible positions for the tooltip
					top: {
						...
						center: cache.elmOffset.top - cache.ttDim.h/2 + cache.elmDim.h/2 + o.nudgeY,
						flushTop: cache.elmOffset.top + o.nudgeY,
						flushBottom: cache.elmOffset.top + cache.elmDim.h - cache.ttDim.h + o.nudgeY,
